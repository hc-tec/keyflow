...
- 2026-04-16 开发者侧下一阶段任务排期（避免上下文丢失）：
  ...
  - 当前执行顺序：
    ...
    2. 再补 Android Host 真机安装验收 runbook
    3. 再增强 `doctor`
  - 2026-04-16 当前已推进：
  ...
  - 2026-04-17 已完成：
    - `preview-rewrite` starter 已发布到 npm：
      - `@keyflow2/function-kit-template-preview-rewrite@0.1.0`
      - tarball：`https://registry.npmjs.org/@keyflow2/function-kit-template-preview-rewrite/-/function-kit-template-preview-rewrite-0.1.0.tgz`
      - integrity：`sha512-w+I3DlewQKqNjklYDZNOrE9TAUGLIcf0k26PYzV/GvsvTCx4Bqg0+WUTRUK2ygGOVdCbQz7fGNC825Yt7OIiZw==`
    - `create-function-kit` 已发布新版并对外暴露官方模板别名：
      - `@keyflow2/create-function-kit@0.1.2`
      - tarball：`https://registry.npmjs.org/@keyflow2/create-function-kit/-/create-function-kit-0.1.2.tgz`
      - integrity：`sha512-tf/uE5V56rS9xjwhr2pKDQC9LS2z5BN0uK1cMHcLn+XPXFIqyx6orCfmh7DGQEHutZkHx3ButobQ2tHq8MsadA==`
      - `npx -y @keyflow2/create-function-kit@0.1.2 --list-templates` 已可见 `preview-rewrite`
      - `npx -y @keyflow2/create-function-kit@0.1.2 artifacts/smoke/npm-preview-live --template preview-rewrite --kit-id keyflow2.preview-live --name "Preview Live" --force --dry-run` 已通过
    - README 口径已同步：
      - `README.md`
      - `templates/create-function-kit/README.md`
      - 删除“preview-rewrite 尚未发布，需先走 --template-dir”的旧说明
    - 开发者入口收敛决策：
      - 不再继续做 `headless-action` starter；当前判断是通用 starter + AI 改写已经足够，继续拆动作型模板只会增加模板矩阵和维护噪音。
      - 不把 `tmp/build_function_kit_icon_set.py` 内置到各个 starter；它依赖 Python + Pillow，且不属于 starter 的高频首屏闭环。starter 继续保留默认 SVG、`doctor` 图标检查和 manifest 图标字段约束即可。
      - 如果后面真的要产品化图标产线，应单独做成可选工具，而不是塞进每个 starter。
...
- 2026-04-16 catalog 提交流程产品化（开发者自检 + PR 校验 + merge 后自动发布）：
  - 背景：此前 starter 只能生成 `catalog:entry` 片段，但开发者无法在提交前验证已发布 npm 包是否真的可用于官方 catalog；仓库侧也缺少面向 `official.packages.json` 的专用 PR 校验流。
  - starter 新增：
    - `templates/function-kit-template-petite-vue/scripts/catalog-check.mjs`
    - `templates/function-kit-template-preview-rewrite/scripts/catalog-check.mjs`
    - 对应 `package.json` 新增 `npm run catalog:check`
  - `catalog:check` 能力：
    - 到真实 npm registry 校验 `name@version` 是否已发布
    - 下载 tarball 并校验 `dist.integrity`
    - 验证 tarball 内 `manifest.json` / `package.json` / bundle / icon 是否齐全
    - 检查已发布包的 `manifest.id/version/entry.type/platforms/runtimePermissions`
    - 输出：
      - `artifacts/catalog/<kitId>.catalog-check.json`
      - `artifacts/catalog/<kitId>.catalog-entry.md`
  - 仓库侧新增维护/校验脚本：
    - `scripts/npm/add-official-package.mjs`
    - `scripts/npm/validate-official-catalog-submission.mjs`
  - GitHub Actions：
    - 新增 PR 专用校验：`.github/workflows/catalog-submission.yml`
      - 对 `catalog/official.packages.json` 变更执行 base-vs-head diff
      - 重新生成 catalog 做真实校验
      - 上传 summary/report artifact
    - `publish-official-catalog.yml` 触发条件收紧为只在 `catalog/official.packages.json` 变更后自动发布，避免脚本改动误触发 catalog 发版
    - `ci.yml` 的 catalog sanity 改为 `--no-include-downloads`，降低不必要的网络波动
  - 文档同步：
    - `templates/function-kit-template-petite-vue/README.md`
    - `templates/function-kit-template-petite-vue/docs/WORKFLOW.md`
    - `templates/function-kit-template-preview-rewrite/README.md`
    ...
    - `CONTRIBUTING.md`
  - 验证：
    - `node --check templates/function-kit-template-petite-vue/scripts/catalog-check.mjs`
    - `node --check templates/function-kit-template-preview-rewrite/scripts/catalog-check.mjs`
    - `node --check scripts/npm/add-official-package.mjs`
    - `node --check scripts/npm/validate-official-catalog-submission.mjs`
    - `npm --prefix templates/function-kit-template-petite-vue run catalog:check -- --help`
    - `npm --prefix templates/function-kit-template-preview-rewrite run catalog:check -- --help`
    - `node scripts/npm/build-starter-template.mjs`
    - `node scripts/npm/verify-starter-template.mjs`
    - `node scripts/npm/build-preview-rewrite-template.mjs`
    - `node scripts/npm/verify-preview-rewrite-template.mjs`
    - `node scripts/npm/build-create-function-kit.mjs`
    ...
    - `node templates/create-function-kit/bin/create-function-kit.mjs artifacts/smoke/catalog-check-tone --template-dir templates/function-kit-template-petite-vue --kit-id tone-rewrite --name "Tone Rewrite" --force`
    - `npm run catalog:check -- --scope keyflow2 --version 0.2.1`（workdir=`artifacts/smoke/catalog-check-tone`，用真实已发布 `@keyflow2/keyflow-kit-tone-rewrite@0.2.1` 做 smoke）
...
- 2026-04-17 starter npm 发布前置条件收口 + 集成测试：
  - `create-function-kit` 与两个官方 starter 的入口文档现在前置写清：
    - `open:kitstudio` 依赖本机 `kit-studio`，支持 `KITSTUDIO_ROOT / KITSTUDIO_HOST / KITSTUDIO_PORT`
    - `publish:npm` 仍需 npm 账号、目标 package/scope 发布权限，以及 `--token-file` / `NPM_TOKEN` / `NODE_AUTH_TOKEN` / `npm login`
    - `ai.request` 的真实 `Base URL / API key / model` 不走 starter 自带 `.env`，而是在 KitStudio / Android Host 的共享 AI 配置里完成
  - 两个 starter 的 `publish:npm` 现在在非 `--dry-run` 下会先跑 `npm whoami` 预检；无认证时直接 fail fast，不再等到真正 `npm publish` 才报 `ENEEDAUTH`：
    - `templates/function-kit-template-preview-rewrite/scripts/publish-kit-npm.mjs`
    - `templates/function-kit-template-petite-vue/scripts/publish-kit-npm.mjs`
  - 文档同步：
    - `templates/create-function-kit/README.md`
    - `templates/function-kit-template-preview-rewrite/README.md`
    ...
    - `templates/function-kit-template-petite-vue/docs/WORKFLOW.md`
  - 集成测试：
    - `node templates/create-function-kit/bin/create-function-kit.mjs ... --template-dir templates/function-kit-template-preview-rewrite --kit-id keyflow2.integration-preview --name "Integration Preview" --force`
    - `node templates/create-function-kit/bin/create-function-kit.mjs ... --template-dir templates/function-kit-template-petite-vue --kit-id keyflow2.integration-petite --name "Integration Petite" --force`
    - `npm run doctor`（workdir=`tmp/integration-preview-rewrite`）
    - `npm run doctor`（workdir=`tmp/integration-petite-vue`）
    - `npm run pack:npm -- --scope keyflow2`（两个工作区都通过）
    - `npm run publish:npm -- --scope keyflow2 --token-file tmp/npm-token.txt --dry-run`（两个工作区都通过）
    - 无认证 smoke：清空 `NPM_TOKEN/NODE_AUTH_TOKEN` 并隔离 `NPM_CONFIG_USERCONFIG` 后，`publish:npm` 在 `npm whoami` 预检处按预期 fail fast
    - 带 token 非 dry-run smoke：认证通过后，故意对已发布的 `@keyflow2/function-kit-template-preview-rewrite@0.1.0` 发包，最终在 npm 侧被“不能覆盖已发布版本”拒绝，证明认证预检已越过且未误发新版本

- 2026-04-17 starter 默认 npm 包名改为“无 scope 优先”口径：
  - 决策：对外默认示例不再要求 `--scope`；大部分开发者一开始没有 npm org/scope，默认路径直接走 unscoped 包更符合直觉。
  - 口径：
    - `kitId` 示例统一改成 `myname.launchpad` / `myname.proofreader`，明确这只是 kitId 命名空间，不是 npm scope
    - `pack:npm / publish:npm / catalog:check / catalog:entry` 默认示例统一改成不带 `--scope`
    - 文档显式说明：不传 `--scope` 时，包名默认生成为 `keyflow-kit-<kitId>`；只有真的要发 `@org/...` 时，才额外使用 `--scope myorg` 或 `--package-name @myorg/my-kit`
  - 落地文件：
    - `README.md`
    - `CONTRIBUTING.md`
    - `scripts/npm/README.md`
    ...
    - `templates/function-kit-template-preview-rewrite/README.md`
    - `templates/function-kit-template-preview-rewrite/docs/WORKFLOW.md`
    - `templates/function-kit-template-preview-rewrite/docs/ANDROID_HOST_RUNBOOK.md`
    - `templates/function-kit-template-preview-rewrite/scripts/{pack-kit-npm,publish-kit-npm,catalog-check,generate-catalog-entry,rename-starter}.mjs`
    - `templates/function-kit-template-preview-rewrite/workspace/function-kits/preview-rewrite-starter/README.md`
    - `templates/function-kit-template-petite-vue/README.md`
    - `templates/function-kit-template-petite-vue/docs/WORKFLOW.md`
    - `templates/function-kit-template-petite-vue/docs/ANDROID_HOST_RUNBOOK.md`
    - `templates/function-kit-template-petite-vue/scripts/{pack-kit-npm,publish-kit-npm,catalog-check,generate-catalog-entry,rename-starter}.mjs`
    - `templates/function-kit-template-petite-vue/workspace/function-kits/starter-showcase/README.md`
  - smoke：
    - `node templates/create-function-kit/bin/create-function-kit.mjs tmp/scope-default-smoke --template-dir templates/function-kit-template-preview-rewrite --kit-id myname.scope-default --name "Scope Default" --force`
    - 生成后的工作区 README 已确认默认命令不带 `--scope`
    - `npm run pack:npm -- --dry-run`（workdir=`tmp/scope-default-smoke`）输出 packageName=`keyflow-kit-myname.scope-default`
    - `npm run publish:npm -- --dry-run`（workdir=`tmp/scope-default-smoke`）输出 packageName=`keyflow-kit-myname.scope-default`

- 2026-04-18 starter 官方 catalog 提交流程澄清（避免把本地 artifact 当官方提交物）：
  - 背景：用户指出 inline-translate 上架说明把 npm 发布、`catalog:check` 校验、官方仓库 PR 三件事混在了一起；starter 侧需要把真实边界写清楚。
  - 脚本收口：
    - `catalog:entry` 的帮助和输出现在明确标注“只生成本地 helper JSON，不是官方仓库提交文件”；同时补上真正要写入官方仓库的 package spec（`<package>@<version>`），不再只给包名：
      - `templates/function-kit-template-preview-rewrite/scripts/generate-catalog-entry.mjs`
      - `templates/function-kit-template-petite-vue/scripts/generate-catalog-entry.mjs`
    - `catalog:check` 的帮助和生成 Markdown 现在明确写出：必须校验真实已发布 npm 包；官方 PR 只改 `catalog/official.packages.json`；不要提交本地 `artifacts/catalog/*.json`：
      - `templates/function-kit-template-preview-rewrite/scripts/catalog-check.mjs`
      - `templates/function-kit-template-petite-vue/scripts/catalog-check.mjs`
  - 文档同步：
    - `templates/create-function-kit/README.md`
    - `templates/function-kit-template-preview-rewrite/README.md`
    - `templates/function-kit-template-preview-rewrite/docs/WORKFLOW.md`
    - `templates/function-kit-template-preview-rewrite/scripts/rename-starter.mjs`
    - `templates/function-kit-template-petite-vue/README.md`
    - `templates/function-kit-template-petite-vue/docs/WORKFLOW.md`
    - `templates/function-kit-template-petite-vue/scripts/rename-starter.mjs`
  - 新口径：
    - `pack:npm` / `publish:npm --dry-run` 通过，不代表已经可以提交官方 catalog
    - 必须先真实发布 npm，并确认 `npm view <package>@<version>` 与 `npm run catalog:check` 都通过
    - 官方 PR 的真实 diff 只有 `catalog/official.packages.json` 里新增一条 package spec 字符串
    - `catalog:entry` / `artifacts/catalog/*.json` 只用于 PR 描述或 Issue 字段准备，不是官方仓库提交文件
  - 验证：
    - `node --check templates/function-kit-template-preview-rewrite/scripts/generate-catalog-entry.mjs`
    - `node --check templates/function-kit-template-petite-vue/scripts/generate-catalog-entry.mjs`
    - `node --check templates/function-kit-template-preview-rewrite/scripts/catalog-check.mjs`
    - `node --check templates/function-kit-template-petite-vue/scripts/catalog-check.mjs`
    - `node templates/create-function-kit/bin/create-function-kit.mjs tmp/smoke-catalog-clarity-preview --template-dir templates/function-kit-template-preview-rewrite --kit-id myname.catalog-clarity-preview --name "Catalog Clarity Preview" --force`
    - `node templates/create-function-kit/bin/create-function-kit.mjs tmp/smoke-catalog-clarity-petite --template-dir templates/function-kit-template-petite-vue --kit-id myname.catalog-clarity-petite --name "Catalog Clarity Petite" --force`
    - `rg -n "官方 catalog 提交前必须先真实发布 npm 包|catalog/official.packages.json" tmp/smoke-catalog-clarity-preview/README.md`
    - `rg -n "官方 catalog 提交前必须先真实发布 npm 包|catalog/official.packages.json" tmp/smoke-catalog-clarity-petite/README.md`
    - `npm run catalog:entry -- --help`（workdir=`tmp/smoke-catalog-clarity-preview`）
    - `npm run catalog:check -- --help`（workdir=`tmp/smoke-catalog-clarity-preview`）
    - `npm run catalog:entry -- --help`（workdir=`tmp/smoke-catalog-clarity-petite`）
    - `npm run catalog:check -- --help`（workdir=`tmp/smoke-catalog-clarity-petite`）

- 2026-04-18 inline-translate 加入官方 catalog 并发布新版 catalog 包：
  - npm 已确认存在并通过 starter catalog 校验：
    - 功能件包：`@keyflow2/inline-translate@0.1.0`
    - kitId：`keyflow2.inline-translate`
    - tarball：`https://registry.npmjs.org/@keyflow2/inline-translate/-/inline-translate-0.1.0.tgz`
    - integrity：`sha512-ZNK4FKsDi4Vfa9UKEYnMs/oXRMmodM/rROHlIhFZyBmskv1p4ZrQY8EX3zTn1Ug+3U+D5uWulvQN0PIeWpthrQ==`
  - 官方包列表已新增真实 npm package spec：
    - `catalog/official.packages.json`
  - 官方 catalog 已重新生成，包含 inline-translate 条目和 sidecar 预览图标：
    - `catalog/official.catalog.json`
    - `catalog/official.catalog.assets/icons/keyflow2.inline-translate/icons/icon-128.png`
  - GitHub Actions 自动发布流程补齐 sidecar assets 提交，避免后续只提交 `official.catalog.json` 漏掉图标目录：
    - `.github/workflows/publish-official-catalog.yml`
  - 已发布新版官方 catalog 包：
    - npm：`@keyflow2/keyflow-kit-catalog@0.0.10`
    - tarball：`https://registry.npmjs.org/@keyflow2/keyflow-kit-catalog/-/keyflow-kit-catalog-0.0.10.tgz`
    - integrity：`sha512-k5JuesSaFnEqeAQOGugkG9+nGd6CqPRpz7XFtR8EZWuzf5QNiViA6MRy75FxlZL5WU4GVbpk26HP06JF57IR3Q==`
  - 验证：
    - `npm view @keyflow2/inline-translate@0.1.0 name version dist.tarball dist.integrity --registry https://registry.npmjs.org/ --json`
    - `npm run catalog:check -- --kit keyflow2.inline-translate --package-name @keyflow2/inline-translate --version 0.1.0`（workdir=`D:\AIProject\my-kits\my-inline-translate`）
    - `node scripts/npm/generate-catalog-from-registry.mjs --packages-file catalog/official.packages.json --out-file catalog/official.catalog.json`
    - `node scripts/npm/validate-official-catalog-submission.mjs --base-ref origin/main`
    - `node scripts/npm/publish-catalog-package.mjs --catalog catalog/official.catalog.json --name @keyflow2/keyflow-kit-catalog --token-file tmp/npm-token.txt`
    - `node scripts/npm/verify-npm-catalog.mjs --pkg @keyflow2/keyflow-kit-catalog@0.0.10`
    - `npm view @keyflow2/keyflow-kit-catalog@0.0.10 name version dist.tarball dist.integrity --registry https://registry.npmjs.org/ --json`

- 2026-04-17 去掉功能件界面中的“向上扩张 / 向下缩小”按钮与行为：
  - Android Host 的 Function Kit 面板工具条不再显示 expand/collapse 图标。
  - 同时移除宿主侧的手动放大/收起逻辑，以及打开功能件时自动进入放大态的行为；现在功能件面板只保留默认高度，不再额外切换窗口高度。
  - 相关文件：
    - `TODO/ime-research/repos/fcitx5-android/app/src/main/java/org/fcitx/fcitx5/android/input/functionkit/FunctionKitWindow.kt`
  - 验证：
    - `rg 'panelExpandButton|panelExpanded|togglePanelExpanded|expandPanel|collapsePanelExpanded' .../FunctionKitWindow.kt`（无残留引用）
    - `.\gradlew.bat :app:compileStandardDebugKotlin --no-daemon`（workdir=`TODO/ime-research/repos/fcitx5-android`）

- 2026-04-17 功能件输入框返回主键盘后按键高度缩小修复：
  - 根因：功能件输入框激活后，嵌入键盘复用的是同一个 `KeyboardWindow` view；detach 期间如果先把功能件窗口高度压回面板高度，部分设备会把这个缩小测量结果带回主键盘。
  - 修复：
    - `FunctionKitWindow` detach 时不再触发嵌入键盘布局缩高，改为移除嵌入键盘后直接恢复进入功能件前的键盘高度。
    - `KeyboardWindow` 直接挂回主键盘窗口时主动按当前键盘高度配置恢复 `InputWindowManager` 高度；嵌入功能件时不会触发该恢复。
    - `FunctionKitBindingsWindowController` 同步规避搜索页 detach 时先失焦造成的同类缩高路径。
  - 相关文件：
    - `TODO/ime-research/repos/fcitx5-android/app/src/main/java/org/fcitx/fcitx5/android/input/functionkit/FunctionKitWindow.kt`
    - `TODO/ime-research/repos/fcitx5-android/app/src/main/java/org/fcitx/fcitx5/android/input/functionkit/FunctionKitBindingsWindowController.kt`
    - `TODO/ime-research/repos/fcitx5-android/app/src/main/java/org/fcitx/fcitx5/android/input/keyboard/KeyboardWindow.kt`
...
- 2026-04-18 KitStudio Packages 不再默认假设手机能访问电脑 URL：
  - 问题：KitStudio 原先在 Packages / Catalog 里主推 Catalog URL / ZIP URL；这隐含手机和电脑同局域网且手机能访问电脑端口，实际经常不成立。
  - 修复：
    - Packages 列表把「下载 ZIP」改成每个 kit 的主操作，普通 Web 版直接触发浏览器下载。
    - Electron 版因安全策略阻止浏览器下载，新增安全 IPC 保存通道：只允许保存同源 `/api/kit-packages/<kitId>.zip`，弹出保存对话框后由主进程拉取本机 server zip 并写入磁盘。
    - UI/README/TODO 文案改为：ZIP 文件安装是推荐路径；Catalog URL / ZIP URL 只保留为同局域网调试备选。
  - 相关文件：
    - `TODO/ime-research/repos/kit-studio/web/app.js`
    - `TODO/ime-research/repos/kit-studio/web/index.html`
    - `TODO/ime-research/repos/kit-studio/src/electron/main.mjs`
    - `TODO/ime-research/repos/kit-studio/src/electron/preload.cjs`
    - `TODO/ime-research/repos/kit-studio/README.md`
    - `TODO/ime-research/repos/kit-studio/TODO.md`
  - 验证：
    - `node --check web/app.js`
    - `node --check src/electron/main.mjs`
    - `node --check src/electron/preload.cjs`
    - 临时启动 `KITSTUDIO_PORT=39091 node src/server.mjs` 后，请求 `/api/kit-packages` 并下载 `bridge-debugger.zip` 成功（11587 bytes）

- 2026-04-18 KitStudio UI 彻底重构（Run/Input/Inspect/Ship 主线）：
  - 背景：用户明确指出当前 KitStudio 是“胡乱堆砌 + 太多解释性小字”，需要从信息架构和视觉语言两层一起重做，而不是只换皮。
  - 结构重排：
    - 左栏改成 `Mission Control -> Workbench -> Input Lab -> Ship ZIP` 主路径，`Permissions / Context / Remote Attach` 退到次级区
    - 中栏统一为 `Stage`，突出正在运行的 kit 与预览设备
    - 右栏统一为 `Signals`，强调“看发生了什么”，而不是把所有工具平铺成 DevTools 拼盘
  - 表达收口：
    - `Packages / Catalog` 改成 `Ship ZIP`，ZIP 下载成为绝对主操作，LAN URL 仅保留为备选
    - Host Input Lab 的“下一步说明”改成简短 action hint，不再用大段教学式小字
    - `Remote Attach` 改成 off-path 实验区语义，避免和本地主链路抢注意力
  - 视觉系统：
    - 从旧的深蓝表单堆叠改成暖色 control-room/workbench 风格，统一字体、圆角、层次、舞台网格与卡片层级
    - Package 卡片在窄左栏下改成纵向布局，修掉按钮被挤出可视区与横向滚动条问题
  - 排障：
    - 发现 39001 端口上残留旧 `kit-studio` server，e2e 一度在测旧页面；已停掉旧进程并在当前工作区重新跑
    - `scripts/e2e.ps1` 对 Shift+Enter 的断言从旧 hint 文案解耦，改成检查真实状态，避免 UI 文案压缩后误报
  - 涉及文件：
    - `TODO/ime-research/repos/kit-studio/web/index.html`
    - `TODO/ime-research/repos/kit-studio/web/styles.src.css`
    - `TODO/ime-research/repos/kit-studio/web/styles.css`
  ...
  - 验证：
    - `npm run css:build`（workdir=`TODO/ime-research/repos/kit-studio`）
    - `node --check web/app.js`（workdir=`TODO/ime-research/repos/kit-studio`）
    - `node --check src/server.mjs`（workdir=`TODO/ime-research/repos/kit-studio`）
    - `npm run e2e`（workdir=`TODO/ime-research/repos/kit-studio`，最终 PASS）


- [x] 去掉功能件界面中的，向上扩张向下缩小的图标和功能


- [x] 我发现一个高度bug，默认的高度配置是正常的，当我进入到功能件页面，然后激活输入框，接着返回到主键盘界面，就会发生键盘按键的大小缩小的问题。

- [x] kitstudio提供的package url压根不能用啊，手机和电脑不在一个区域网，或者说你不能断定就在一个局域网啊，你应该提供打包后的zip给用户
