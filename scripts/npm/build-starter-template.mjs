...
const outRoot = path.join(repoRoot, "artifacts", "npm", "templates", "function-kit-template-petite-vue");
...
if (!fileName) {
  throw new Error("[npm] npm pack did not return a filename for the starter template.");
}
...
await writeJson(path.join(outRoot, "starter-template.json"), {
...
console.log(`[npm] starter template: ${pkg.name}@${pkg.version}`);
